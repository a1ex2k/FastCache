...
namespace FastCache
{
    ...
    public class FastCache<TKey, TValue> : IEnumerable<KeyValuePair<TKey, TValue>>, IDisposable
    {
        ...
        private readonly long _slidingExpirationInterval;

        /// <summary>
        /// Initializes a new empty instance of <see cref="FastCache{TKey,TValue}"/>
        /// </summary>
        /// <param name="cleanupJobInterval">cleanup interval</param>
        /// <param name="slidingInterval">interval that added when getting value (<see cref="TimeSpan.Zero"/> to disable sliding expiration</param>
        public FastCache(TimeSpan cleanupJobInterval, TimeSpan slidingInterval)
        {
            _cleanUpTimer = new PeriodicTimer(cleanupJobInterval);
            _slidingExpirationInterval = (long)slidingInterval.TotalMilliseconds;
            Task.Run(() => EvictExpiredJob());
        }
        ...
        public bool TryGet(TKey key, out TValue value)
        {
            ...
            if (_slidingExpirationInterval > 0)
            {
                ttlValue.SlideExpiration(_slidingExpirationInterval);
            }
        ...
        private class TtlValue
        {
            ...
            public long TickCountWhenToKill { get; private set; }
            ...
            public void SlideExpiration(long ticks)
            {
                TickCountWhenToKill = Environment.TickCount64 + ticks;
            }
